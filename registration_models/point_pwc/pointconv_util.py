...
class PointConv(nn.Module):
    def __init__(self, nsample, in_channel, out_channel, weightnet=16, bn=use_bn, use_leaky=True, norm='bn'):
        ...
        if bn:
            if norm == 'bn':
                ...
                self.bn_linear = nn.InstanceNorm1d(out_channel)
            else:
...
class SceneFlowEstimatorPointConv(nn.Module):
    ...
    def __init__(self, feat_ch, cost_ch, flow_ch=3, channels=[128, 128], mlp=[128, 64], neighbors=9, clamp=[-200, 200],
                 use_leaky=True, use_bn=True, norm='bn'):
        ...
        for _, ch_out in enumerate(channels):
            pointconv = PointConv(neighbors, last_channel + 3, ch_out, bn=use_bn, use_leaky=True, norm=norm)
